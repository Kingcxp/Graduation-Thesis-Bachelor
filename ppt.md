...
GPU 的运行依赖于复杂的层级协作：
1. **应用层**：PyTorch / 游戏引擎。
2. **用户态驱动 (UMD)**：封装复杂的渲染/计算指令（如 CUDA Runtime）。
3. **内核态驱动 (KMD)**：**本文研究核心**。负责权限控制、显存分配、硬件调度。
4. **硬件层**：物理 GPU。

---

![](./assets/gpu-workflow.png)

---

### 🧐 为什么内核态至关重要？

- 在用户态，每个进程都有自己的“虚拟地址空间”，A 进程看不到 B 进程的数据。但在内核态，**所有内核代码共享同一个地址空间**。如果 GPU 驱动在内核态由于漏洞导致内存泄漏，它不仅可能破坏显卡数据，还可能顺手抹掉操作系统的调度表、内存管理单元（MMU）甚至是其他进程的敏感数据（如密码缓存）。
- 内核态可以执行所有 CPU 指令，包括关闭中断、修改页表、直接读写物理端口等。用户态程序如果尝试执行这些指令，会被 CPU 直接拦截并抛出异常；但在内核态，没有任何人能拦截这些指令。
