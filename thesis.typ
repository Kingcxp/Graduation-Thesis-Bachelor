...
#abstract(
  ...
  在异构计算成为算力核心支撑的当下，GPU 已从传统图形渲染场景，全面延伸至深度学习、高性能计算、科学仿真、自动驾驶等关键领域。GPU 驱动作为衔接上层应用与底层硬件的核心中间件，其安全性、稳定性与可靠性直接决定了上层全栈系统的正常运行。由于 GPU 驱动兼具用户态 runtime 与内核态模块的复杂架构，代码规模庞大、逻辑分支密集、硬件相关的状态机复杂度极高，加之 NVIDIA、AMD 等主流厂商的核心驱动均为闭源形态，这使得以 DrChecker @machiry2017drchecker 为代表的传统内核白盒测试与静态分析方法失去了源码分析的基础。虽然学术界曾提出如 Digtool @pan2017digtool 等基于虚拟化的闭源驱动检测框架，但它们主要针对状态简单的普通外设，难以应对 GPU 极高的异步交互复杂度。
